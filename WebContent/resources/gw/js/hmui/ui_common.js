...
$(function(){
  ...
  // readonly input 
  $(document).on('click', '.wrap_inp.readonly label, .inp[readonly]', function(e){
...
var registUI = function(){
  var setSafeArea = new Promise(function (resolve, reject) {
    setTimeout(function(){
      _setSafeArea();
      if (safeArea.top !== 0) {resolve(_setSafeArea());}
      else {reject(_setSafeArea());}
    },200);
  });
  setSafeArea
    .then(function(setValue){_setFixedDefault();})
    .catch(function(setValue){console.log(setValue);});

  if ( $('#header').length ) { _headerControl(); } // 스크롤에 따른 Header
  if ( $('.wrap_contents .section_tab.fnFixedTop').length ) { _fixedTabInPage(); } // 상단고정 기본 탭 초기값 설정
  if ( $('.wrap_contents .fnStickyTop').length ) {$('section[data-roll]').css('overflow-x','unset');} // fnSticky overflow 설정 
  if ( $('.section_bottom_fixed.type_noFullBtn .fnScrollEnd').length ) { _fixedBottomBtnGap(); } // 스크롤에 따른 페이지 하단고정 위치
  if ( $('.contents').find('.section_bottom_fixed .wrap_btn_full').length) { _checkfixedButton(); } // 하단고정버튼 유무에 따른 레이아웃
  ...
  if ( $('.type_won_button.type_interaction').length ) { wonInpInteractionBtn(); } // 네이티브 키패드 호출 인풋모양 버튼 인터렉션 타입
...
var _setSafeArea = function() {
  ...
  $(".wrap_moneybag_tab").length
    ? safeArea.filter = parseInt($(".wrap_moneybag_tab").attr('data-height')) // nrc 조회 필터
    : safeArea.filter = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--filterH'));

  return safeArea;
}

var _setFixedDefault = function(){
  if ( $('.walking-alim-box').next('.fnFixedTop').length ) { _fixedTopInPage(); } // 알림박스 있는 페이지 스크롤에 따른 콘텐츠 상단고정
  if ( $('.wrap_contents .fnStickyTop').length ) { fnStickyTop.set(); } // 스크롤에 따른 상단 sticky 고정
  if ( $('.wrap_contents .fnStickyTop.type_round').length ) { _fixedTopRound(); } // 스크롤에 따른 상단 round sticky 고정
}
  ...
  function _headerChange(el) {
    ...
    $(el).on('scroll', function(e) {
      if ($(this).scrollTop() > 0) {
        ...
        if (!(isiOS && $('body').hasClass('overflow')) && $(el).find('.section_tab.fnFixedTop').length < 1) {
...
/**
 * @name _fixedTabInPage()
 * @description 상단고정 기본 탭 초기값 설정
 */
var _fixedTabInPage = function() {
  var $fixedEl = $('.wrap_contents').find('.section_tab.fnFixedTop');
  var $header = $fixedEl.closest('.wrap_contents').siblings('#header').find('.inner_fixed');
  // var setHeight = safeArea.top + safeArea.header + parseInt($fixedEl.attr('data-height'));

  if (($('.walking-alim-box').css('display') == 'block')) {
    $fixedEl.removeClass('scrolled');
    $fixedEl.next('div').attr('style','');
    $header.parent().removeClass('scrolled fixedTab');
    $header.attr('style','');
    return;
  }
  if ( ($fixedEl.attr('data-addMargin') !== "false") ) {
    $header.parent().addClass('scrolled');
    if (!$fixedEl.hasClass('type_round')) {
      $header.parent().addClass('fixedTab');
    } else {
      $header.parent().addClass('fixedRoundTab');
    }
    // $header.css({'height': setHeight * 0.1 + 'rem',});
    $fixedEl.next('div').css({'margin-top': parseInt($fixedEl.attr('data-height')) * 0.1 + 'rem'});
  }
}

/**
  * @name _fixedTopInPage()
  * @description 알림박스 있는 페이지 스크롤에 따른 콘텐츠 상단고정
  */
var _fixedTopInPage = function() {
  var alimBox = $('.walking-alim-box');
  var $fixedEl = alimBox.next('.fnFixedTop');
  var $header = $fixedEl.closest('.wrap_contents').siblings('#header').find('.inner_fixed');
  var setHeight = safeArea.top + safeArea.header + parseInt($fixedEl.attr('data-height'));
...
/**
  * @name _fixedTopRound()
  * @description 스크롤에 따른 상단 round sticky 고정
  */
var _fixedTopRound = function() {
  if ( iosV() < 13 || parseInt(androidV(ua)) < 8 ) {
  ...
  // Round tab에서 한번더 감지
  const element = document.querySelector('.sticky_sentinel_top');
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          $('.type_round').removeClass('fixed');
          if (toggleClass && toggleClass !== undefined) {
            $fixedEl.find("ul").addClass(toggleClass);
          }
        } else {
          if($('.type_round').hasClass('fixed') === false) {
            $('.type_round').addClass('fixed');
            if (toggleClass && toggleClass !== undefined) {
              $fixedEl.find("ul").removeClass(toggleClass);
            }
          }
        }
      });
  ...
  // round tab sticky 시 header 조정
  var $fixedEl = $('.wrap_contents .type_round');
  var $header = $fixedEl.closest('.wrap_contents').siblings('#header');
  var toggleClass = $fixedEl.attr('data-toggle-class');

  $(window).on('scroll', function() {
    if ($fixedEl.hasClass('fixed')) {
      $header.addClass('fixedRoundTab');
      if (toggleClass && toggleClass !== undefined) {
        $fixedEl.find('.wrap_tab_btn').removeClass(toggleClass);
      }
    } else {
      $header.removeClass('fixedRoundTab');
      if (toggleClass && toggleClass !== undefined) {
        $fixedEl.find('.wrap_tab_btn').addClass(toggleClass);
      }
    }
  });
};
...
function _setFixedTop(fixedEl, header, setHeight) {
  ...
  if ( $(fixedEl).attr('data-addMargin') !== "false" && !$(fixedEl).hasClass('type_round fnStickyTop') ) {
...
var fnStickyTop = (function() {
  ...
  function _addSentinel(el) {
    var elTop = (parseInt($(el).css('top')) - 1),
        sentinelTop = (elTop + safeArea.top + safeArea.header) * 0.1;

    var sentinel = document.createElement('div');
    sentinel.classList.add('sticky_sentinel', 'sticky_sentinel_top');
    if ( $(el).hasClass('wrap_filter')) {
      // 필터
      sentinelTop = sentinelTop - (parseInt($(el).parent('.section_list_trans').css('padding-top')) * 0.1)
    }
    if (container !== $(stickyEl).parents('.wrap_contents')[0]) {
      // 팝업
      var tab = parseInt($(container).find('.fnFixedTop').attr('data-height'))*0.1;
      sentinelTop = sentinelTop + 0.1 + tab;
      sentinel.style.cssText = 'height:'+ tab +'rem; top:'+ -sentinelTop +'rem;';
    } else {
      // 페이지
      sentinel.style.cssText = 'top:'+ -sentinelTop +'rem;';
    }
...
var _checkfixedButton = function() {
  ...
  $fixedEl.parents('.contents').addClass('hasFixedBtn');
}
...
var _tabHightlight = function() {
  $(document).off('click').on('touchstart', '.wrap_link_list a', function(e){
    if (!$(this).hasClass('wrap_box')) {
      $(this).addClass('active');
    }
  });
  $(document).off('click').on('touchend', '.wrap_link_list a', function(e){
...
/**
  * @name wonInpInteractionBtn()
  * @description // textarea 엔터키 입력 방지
  */
var wonInpInteractionBtn = function(){
  $('.type_interaction .btn_won_keypad').on('click', function(e){
    var wrapInp = $(this).closest('.wrap_inp');
    wrapInp.addClass('focused');
  });
}
...
var _quickSelControl = function(el) {
  $(el).each(function(){
    ...
    $wrapSel.find('.btn_acco').off('click').on('click', function(){
      _quickSelCenter($wrapList,listH, 0);
    ...
    $wrapSel.find('.fnOptQuick').off('click').on('click', function(){
      $(this).closest('li').addClass('selected').siblings().removeClass('selected');
...
var _dropDown = function() {
  $('.wrap_dropdown').each(function() {
    ...
    $btnDropDown.off('click').on('click', function(e) {
      if ($el.hasClass('on')) {
        $el.removeClass('on');
      } else {
        $el.addClass('on');
...
var _simpleAcco = function() {
  ...
  accoBtn.off('click').on('click',function(){
...
var _faqAcco = function() {
  $(document).off('click').on('click', '.section_faq .btn_acco', function(e){
    var $itemparent = $(this).closest("ul");
...
var _agreeCheckAll = function() {
  $('.section_terms').each(function(){
    ...
    chks.on('change', function(){
      ...
      // 하위체크
      var sub = $(this).closest('li').find('input[type="checkbox"]'); // 같은 depth 내 체크 개수
      ...
      var sameDep = $(this).closest('.chk_list').children('li'),
          checkLength = sameDep.find('input[type="checkbox"]:checked').length, // 같은 dep 내 체크 갯수
          reqLength = sameDep.find('input[required]').length, // 필수 갯수
          reqChecked = sameDep.find('input[required]:checked').length; // 필수 체크 갯수
      if ( reqLength && (reqLength !== reqChecked) ) { // 필수 체크가 모두 체크 되어야 상위 체크
        parentChk.prop('checked',false);
      } else if ( reqLength <= 0 && checkLength <= 0) { // 필수 체크가 없는 경우 모두 체크 되어야 상위 체크
        parentChk.prop('checked',false);
      } else {
...
var _tooltip = function() {
  ...
  $('.fnOpenTooltip').off('click').on('click', function(){
    $focusEl = $(this);
  ...
  $('.fnCloseTooltip').off('click').on('click', function(){
...
var fnCloseLayerPop = function(popID, focusEl){
  ...
  setTimeout(function(){
    ...
    if (isiOS) {
      $('body').removeClass('iOSLayerOpened');
      $('body').find('#root').attr('style','');
...
var fnToastPop = function(popID) {
  ...
  var fullPop = $('.wrap_layer.show');

  var _openToast = function() {
    clearToastStatus = 'open';
    $el.addClass('show');
    clearToastClass = setTimeout(_closeToast, 3000);
  }
  ...
  if ($el.hasClass('type_snackbar')) {
    ...
    if (clearToastStatus !== 'open') {
      _openToast();
    } else {
      clearTimeout(clearToastClass);
      $('.wrap_toast').removeClass('show closing');
      setTimeout(function(){
        _openToast();
      }, 100);
    }
...
var addNotchMeta = function() {
  var $viewport = $('meta[name="viewport"]');
  if (isiOS) {
    $viewport.attr('content', 'width=device-width, initial-scale=1.0, minimum-scale=1.0, shrink-to-fit=no, viewport-fit=cover');
    $viewport.after('<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent" />');
    $viewport.after('<meta name="apple-mobile-web-app-capable" content="yes" />');
  } else {
    $viewport.after('<meta name="mobile-web-app-capable" content="yes" />');
  }
}
/**
